...
public class SetBindingsListener implements BGMEventListener {
	...
	public void newValueBinding(BGMEvent e) {
		...
		if (b == null) {
			...
			} else if (linkedControl && (e.getVarName().compareTo("sourceColumn") == 0)) {
				linkedControl = false;
	...
	public void newBinding(BGMEvent e) {
		...
		if (b == null) {
			...
			if (e.getVarName().compareTo("operatorName") == 0 || e.getVarName().toLowerCase().endsWith("endtranstime")
					|| e.getVarName().toLowerCase().endsWith("starttranstime") || e.getVarName().compareTo("executed") == 0
			...
			if (binding != null) {
				...
				for (Entry<String, BGMEvent> eventPair : binding.getBindings()) {
					...
					if (eventPair.getKey().compareTo("value") == 0) {
						...
						if (e.getVarName().toLowerCase().endsWith("starttranstime") || e.getVarName().toLowerCase().endsWith("endtranstime")) {
							String fecha=null, hora=null, aux=null;
							tv.setType(pf.getName().XSD_DATETIME);
							Timestamp ts= java.sql.Timestamp.valueOf(e.getValue());
							Date d= new Date(ts.getTime());
							aux=sdf.format(d);
							fecha= aux.substring(0, aux.indexOf(" "));
							hora= aux.substring(aux.indexOf(" ")+1,aux.length());
							tv.setValue(fecha+"T"+hora);
						} else {
	...
	private synchronized void findAndExpand(String idBinding, String nameTemplate, DBCollection coll,
		...
		try {
			...
			if (binding != null && intFr != null) {
				...
				if (d != null) {
					intFr.writeDocument(bout, ProvFormat.TURTLE, d);
